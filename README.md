...
Poi, **senza comprimere niente**: nella pagina **Dental**, pulsante **Import folder**.

- **Choose files…** apre il pannello del browser: scegli i `.dcm` — ⌘A per prenderli tutti.
- **Choose folder…** prende una cartella intera, sottocartelle comprese.
- Nome del paziente, **Import**. I file si caricano uno per volta e il pannello dice a che punto è
  («142 di 600»); poi l'importazione va da sé.

Per una cartella molto grande già sul computer c'è anche la riga *A very large folder*: si incolla
il percorso e il server la legge dal disco invece di caricarla. In nessuno dei due casi qualcosa
esce dal computer.

La strada dell'originale resta, e serve per i `.nii` o per un archivio già pronto: **Import MRI**
vuole uno **ZIP** — senza password, **un paziente per archivio**, fino a 2 GB. In tutti i casi la
conversione gira in un processo a parte: la finestra si può chiudere.
