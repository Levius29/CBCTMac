# La riserva Swift (già `README.md`)

> Questo documento era il `README` del progetto finché il programma era l'applicazione SwiftUI.
> Da quando la base è OpenMRI — vedi [`openmri-dental.md`](openmri-dental.md) — i moduli Swift non
> sono più il prodotto: sono **la riserva**, cioè il magazzino di matematica dentale già provata
> da cui si porta nel programma web una funzione per volta. Il testo resta com'era, perché
> descrive esattamente ciò che c'è in `Sources/` e come lo si compila; cambia solo il suo posto
> nella catena.
